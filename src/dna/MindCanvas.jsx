...
import { REGIONS, dnaBus } from "./dna.js";
...
// loop: dnaBus events stage node flares and particles that travel each
// synapse's bezier by manual quadratic interpolation — no getPointAtLength in
// the hot path, no allocation storms (fixed particle pool, reused arrays).
...
const TOAST_TOP = 62;   // px offset for the activation toast — clears the DnaView header band
...
const REPULSE = 3400;                    // pairwise push ∝ 1/d², capped below (raised within tolerance so hub clusters keep labels clear)
...
.dna-canvas .dna-droptar .dna-core { stroke: ${T.amber} !important; stroke-width: 2.5px !important; }
.dna-canvas g[data-dna-node].dna-fire .dna-core { stroke: ${T.greenHi} !important; }
.dna-canvas g[data-dna-edge].dna-fire .dna-evis { stroke: ${T.greenHi} !important; opacity: 0.95 !important; }
/* Keyboard path — nodes are tabbable; the amber ring only shows for keyboard
...
@media (max-width: 860px) {
  /* App's mobile touch-target floor — a miss on a 26px HUD button falls
     through to the svg and pans the viewport. */
...
function MindCanvasImpl({
  ...
  if (!st.current) {
    ...
    for (let i = 0; i < SPECK_N; i++) {
      specks.push({
        ...
        r: 0.5 + Math.random(), o: 0.04 + Math.random() * 0.05,   // faint ink dust on white
      });
    }
    const S0 = {
      sim: null,
  ...
  // affected path (nodes + fired synapses go emerald), then everything resets.
  ...
  return (
    ...
    >
      ...
      >
        <defs>
          {/* Soft outer glow — one blur filter, colored by the region-tinted
              circle it's applied to (light theme: a lit halo in the region hue,
              not the dark-mode radial-to-transparent orb). */}
          <filter id="dnaGlow" x="-75%" y="-75%" width="250%" height="250%" colorInterpolationFilters="sRGB">
            <feGaussianBlur stdDeviation="5" />
          </filter>
        </defs>

        <g ref={worldRef} className="dna-world">
          {/* Ambient dust — faint ink specks; skipped entirely under reduced motion. */}
          {!RM && (
          ...
          {/* Synapses — emerald excitatory, red dashed inhibitory; width + opacity
              carry the weight. Each gets an invisible fat twin for hit testing. */}
          {g.edges.map((e) => {
            const w = typeof e.weight === "number" ? e.weight : 0.5;
            const inhib = e.polarity === -1;
            const sw = 0.8 + 2.6 * w;                     // width 0.8 → 3.4 by weight
            const bothOn = enabledOf[e.from] !== false && enabledOf[e.to] !== false;
            const op = (0.22 + 0.28 * w) * (bothOn ? 1 : 0.25);  // 0.22 → 0.5 (min 0.22), dimmed if an endpoint is silenced
            ...
            return (
              <g key={e.id} data-dna-edge="" data-id={e.id} ref={edgeRefCb(e.id, "g")} style={{ display: hidden ? "none" : undefined }}>
                {selE && (
                  <path ref={selEdgeRefCb(e.id)} fill="none" stroke={T.amber} strokeWidth={sw + 3.5} opacity="0.3" strokeLinecap="round" style={{ pointerEvents: "none" }} />
                )}
                <path
                  className="dna-evis"
                  ref={edgeRefCb(e.id, "vis")}
                  fill="none"
                  stroke={inhib ? T.red : T.green}
                  strokeWidth={sw}
                  opacity={op}
          ...
          {/* Nodes — soft glow, flare ring (pulse-driven), selection halo, solid
              region core, padlock for locked governance, hover "+" port, label. */}
          {g.nodes.map((n) => {
            const hidden = visSet ? !visSet.has(n.id) : false;
            const r = nodeR(n.weight);
            const regKey = REGIONS[n.region] ? n.region : "identity";
            const regColor = REGIONS[regKey].color;
            ...
            return (
              ...
              >
                ...
                {/* soft outer glow — region color, blurred, low opacity */}
                <circle r={r} fill={regColor} opacity={disabled ? 0 : 0.32} filter="url(#dnaGlow)" style={{ pointerEvents: "none" }} />
                <circle className="dna-flare" ref={flareRefCb(n.id)} r={r + 4} fill="none" stroke={T.greenHi} strokeWidth="2" opacity="0" style={{ pointerEvents: "none" }} />
                {selN && (
                  <>
                    <circle r={r + 9} fill="none" stroke={T.amber} strokeWidth="5" opacity="0.16" style={{ pointerEvents: "none" }} />
                    <circle r={r + 5.5} fill="none" stroke={T.amber} strokeWidth="1.4" opacity="0.95" style={{ pointerEvents: "none" }} />
                  </>
                )}
                {/* core — solid region fill + 1px ink outline */}
                <circle className="dna-core" r={r} fill={regColor} stroke={T.nodeStroke} strokeWidth="1" />
                {n.locked && (
                  <g transform={`translate(${r1(r * 0.72)} ${r1(-r * 0.72)})`} style={{ pointerEvents: "none" }}>
                    <circle r="5.4" fill="rgba(255,255,255,0.92)" stroke={T.amberLine} strokeWidth="0.8" />
                    <path d="M -1.7 -0.4 v -0.9 a 1.7 1.7 0 0 1 3.4 0 v 0.9" fill="none" stroke={T.amber} strokeWidth="1.1" strokeLinecap="round" />
                    <rect x="-2.3" y="-0.5" width="4.6" height="3.5" rx="0.9" fill={T.amber} />
                  </g>
                )}
                <circle className="dna-port" data-dna-port="" data-id={n.id} cx={r1(r + 7)} cy="0" r="5.5" fill={T.surface} stroke={T.amberLine} strokeWidth="1" />
                <path className="dna-port" d={`M ${r1(r + 4.5)} 0 h 5 M ${r1(r + 7)} -2.5 v 5`} fill="none" stroke={T.amber} strokeWidth="1.2" style={{ pointerEvents: "none" }} />
                <text
                  className="dna-label"
                  y={r1(r + 15)}
                  textAnchor="middle"
                  style={{ fontFamily: T.fontDisplay, fontSize: "10px", fontWeight: 600, fill: T.ink, pointerEvents: "none", paintOrder: "stroke", stroke: T.bg, strokeWidth: "3px", strokeLinejoin: "round" }}
                >
                  {n.label}
                </text>
              </g>
          ...
          {/* Particle pool — fixed set of emerald comet pairs, recycled across pulses. */}
          {!RM && (
            <g style={{ pointerEvents: "none" }}>
              {POOL_IDX.map((i) => (
                <g key={i}>
                  <circle ref={S.poolTRefCbs[i]} r="1.6" fill={T.greenHi} opacity="0" />
                  <circle ref={S.poolHRefCbs[i]} r="2.5" fill={T.greenHi} opacity="0" />
                </g>
              ))}
            </g>
          )}
      ...
      {toast && (
        <div
          style={{
            ...glass,
            position: "absolute", top: `${TOAST_TOP}px`, left: "50%", transform: "translateX(-50%)",
            ...
            padding: "7px 16px", borderRadius: T.rPill, border: `1px solid ${T.amberLine}`,
      ...
      {/* HUD — white-glass pill cluster, bottom-right. Small enough to never block gestures. */}
      <div
        style={{
          ...glass,
      ...
      >
        ...
        <button
          ...
          style={{ color: physOn ? T.amber : T.sub, fontSize: "10px" }}
        >
          {physOn ? "❚❚" : "▶"}
        </button>
      </div>
...
// memo so the host view's worker poll (and every keystroke in its Pulse input)
// never re-reconciles the ~650-element SVG tree — with the host's callbacks
// stable, this component re-renders only on genome/selection/filter changes and
// its own toast. Bus-driven animation bypasses React entirely.
